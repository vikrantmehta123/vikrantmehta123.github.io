...
The problem is straightforward. The easiest way to solve this is by sorting the list in descending order and then selecting `k`th element in the list. But we're told that we cannot use sorting. 
...
- Among these `k` largest elements, `k`th largest element, is the smallest element (if you're given a list of `k` elements and you want to find `k`th largest element, that's the same as finding the minimum in the list). So at the end, we can pop the smallest element from the min-heap, and return it.
...
### 7. [Maximum Level Sum of a Binary Tree](https://leetcode.com/problems/maximum-level-sum-of-a-binary-tree/description/)
