...
It's incredible how simple this looks. Let's unpack the code:

- We sort the `citations` array because we want the maximum number of citation that fulfills some condition.
- What does `i` in the `for` loop mean? It's the number of citations that are greater than the current one. 

That's it! We wanted the  the maximum value of `h` such that there are least `h` papers that have each been cited at least `h` times.
